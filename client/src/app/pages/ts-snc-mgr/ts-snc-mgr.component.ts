...
@Component({
  selector: 'ngx-ts-snc-mgr',
  templateUrl: './ts-snc-mgr.component.html',
  styleUrls: ['./ts-snc-mgr.component.scss']
})
export class TsSncMgrComponent implements OnInit {
  ...
  ngOnInit(): void {
   this.prepareForm();
    const obj = this.srSPXLst.getMRolIDByRoleName(this.srGlbVar.rolSNCMGR);
    obj.then((rs) => {
      this._pgRolID = rs;
    });
    this.getTsName();
